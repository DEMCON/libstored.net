...
- **LibStored.Net.Generator.Tests**: Contains unit tests for the generator logic.
- **LibStored.Net.IntegrationTest**: A sample project that references the generator, configures `AdditionalFiles`, and verifies that generated code compiles and behaves as expected.
- **LibStored.Net.NugetIntegrationTest**: The generator is also tested by consuming it as a NuGet package in a separate test project, ensuring that installation and usage via NuGet works as expected. Run this seperate test using the `.\test-nuget.ps1` script in the root.
