...
1. **Add Reference**  
   ...
   > The `LibStored.Net.Generator` is packages as source generator into the `LibStored.Net` package.
...
Suppose you define a store metadata file `TestStore.json`:
...
Add the JSON metadata in your `.csproj` as AdditionalFiles:
...
The metadata JSON file must match the structure defined by `StoreModel` in the generator.
