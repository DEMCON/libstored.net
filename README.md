...
- Thread-safe access to the store is not supported (al not in the orignial C++ code). The store may only be accessed from a single thread (the thread that runs the NetMQPoller). Probably a lock will be added around the entire store buffer to make the store thread-safe.
- Only little endian store are supported.
...
#### Store generation
Currently stores are create manually based on the documentation generated by [libstored](demcon.github.io/libstored), the `.rtf` containing the hash, names, types, offset and sizes of all objects in the store. The store need to derive from the (Debug)Store abstract class, such that the Debugger and/or Synchroniser can map it.

A C# SourceGenerator could generate the stores automatically during compile time of the C# application using this library. The input of the source generated is the `.rtf` / `.csv` / `Meta.py`, or parse the actual store definition in `.st` files. Maybe [libstored](demcon.github.io/libstored) can be extended to generate a language agnostic `.json` file with the store definition, which can be used by the C# SourceGenerator to generate the store class.
